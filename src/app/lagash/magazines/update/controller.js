...
export class LagashMagazinesUpdateController {
  ...
  constructor(
    ...
    magazine.tags = magazine.tags ? BasicOption.get_tags(magazine.tags) : [];
    this.item = magazine;

    // autor
    Authors.find_authors({
      material_id: this.magazine_id
    }).$promise
    .then((res) => {
      this.authors = res;
    })
  ...
  create_ejemplar() {
    ...
    this.MagazinesEjemplares.next().$promise
    .then((res) => {
      this.ejemplar_item = {
        _id: this.UUID.next(),
        order: this.getOrder(),
